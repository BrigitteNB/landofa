...
class RegistrationController extends AbstractController
{
        ...
        if ($form->isSubmitted() && $form->isValid())
         {
           ...
           $user->setTokenForEmailVerification($token_generated);
            ...
            $sendEmailService->send([
                "sender_email" => "info@landofa.com",
                "sender_name" => "Landofa",
                "recipient_email"=> $user->getEmail(),
                "subject"=> "Vérification de votre compte sur le site Landofa",
    ...
    public function emailVerif(User $user, string $token, UserRepository $userRepository) : Response
    {
        ...
        if (new \DateTimeImmutable('now') > $user->getExpiresAt())
        {
            ...
            throw new CustomUserMessageAccountStatusException("Votre délai de vérification du compte est dépasé depuis le : $deadline.  Veuillez vous réinscrire.");
        }
